...
const ShowEmployees = ({ employeeData }) => {
  ...
  return (
    <div className="dflex flex-column col-lg-3 gap-3 border border-secondary-subtle h-100 p-3 rounded-2">
      ...
      <output className="my-3 bg-secondary d-flex justify-content-start">
        <ul className="lh-lg text-light mx-1">
          <ol>{details.employeeID}</ol>
          <ol>{details.firstName}</ol>
          <ol>{details.lastName}</ol>
        </ul>
      </output>
    </div>
  );
