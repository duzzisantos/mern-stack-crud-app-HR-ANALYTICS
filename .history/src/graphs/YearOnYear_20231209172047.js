...
const YearOnYear = ({ chartData, employeeId }) => {
  ...
  const uniqueEmployee = chartData
    .filter((el) => employeeId.match(new RegExp(`${el.ID}`) ? el : !el))
    .map((x) => x);

  const uniqueAppraisals = getUniqueAppraisals(chartData, employeeId);

  console.log(uniqueEmployee, uniqueYears, uniqueAppraisals);
  const options = {
    responsive: true,
    plugins: {
      legend: {
