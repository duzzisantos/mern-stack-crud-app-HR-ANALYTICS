...
const LeagueTable = () => {
  ...
  const filteredDataByDepartment = () => {
    return getAppraisal.data.filter(
      (element) =>
        selectedDepartment.includes(element.department) &&
        selectedMonth.includes(element.month) &&
        selectedYear === element.year
    );
  };

  console.log(filteredDataByDepartment());
  console.log(typeof [selectedDepartment, selectedMonth, selectedYear]);
  ...
  return (
    <>
      ...
      >
        ...
        <TopThree filteredData={filteredDataByDepartment()} />
      </Container>
    </>
  );
};
